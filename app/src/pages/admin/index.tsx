...
import { Moon, Sun, MessageCircle, UserCircle2, Crown, Skull, Users, Activity, Shield } from 'lucide-react'
import {
  RpcProvider,
  Contract,
  WalletAccount,
...
const FullPageCenter = styled.div`
  ...
  min-height: 100vh;
  width: 100vw;
  background: radial-gradient(circle at 50% 50%, ${theme.colors.overlay}, transparent);
...
const Header = styled.header`
  ...
  align-items: center;
  padding: 1.5rem;
  background: linear-gradient(to right, ${theme.colors.dark}, ${theme.colors.secondary});
...
const PlayerList = styled(motion.div)`
  margin-bottom: 1rem;
  padding: 1.5rem;
  background: linear-gradient(to bottom, ${theme.colors.secondary}, ${theme.colors.dark});
  border: 1px solid ${theme.colors.primary};
  border-radius: 0.5rem;
  animation: ${glowAnimation} 3s infinite;
`

const PlayerListItem = styled(motion.div)<{ isAlive: boolean; isMafia: boolean }>`
  display: flex;
  align-items: center;
  justify-content: space-between;
  ...
  padding: 1rem;
  background: ${theme.colors.dark};
  border: 1px solid ${props => props.isMafia ? theme.colors.danger : theme.colors.primary};
  border-radius: 0.5rem;
...
const ChatMessage = styled(motion.div)<{ isModerator: boolean }>`
  ...
  padding: 1rem;
  background: ${props => props.isModerator ? theme.colors.primary : theme.colors.dark};
  color: ${props => props.isModerator ? theme.colors.dark : theme.colors.primary};
  border: 1px solid ${theme.colors.primary};
  border-radius: 0.5rem;
  font-size: 0.875rem;
  transition: all 0.3s ease;
...
const RoleCard = styled(motion.div)<{ selected: boolean }>`
  padding: 1.5rem;
  background: ${props => props.selected ? theme.colors.primary : theme.colors.dark};
  color: ${props => props.selected ? theme.colors.dark : theme.colors.primary};
  border: 1px solid ${theme.colors.primary};
  border-radius: 0.5rem;
  cursor: pointer;
  transition: all 0.3s ease;
...
const ModeratorMafiaPortal: React.FC = () => {
  ...
  const [currentPhase, setCurrentPhase] = useState<GamePhase>('Role Assignment')
  ...
  const commitRoles = useCallback(async () => {
    try {
      ...
      if (contract && gameId) {
        ...
        console.log('Roles committed:', players.map(p => `${p.name}: ${p.role}`))
  ...
  const eliminatePlayer = useCallback(async (playerName: string) => {
    try {
      const contract = await getContract()
      if (contract && gameId) {
  ...
  const togglePhase = useCallback(async () => {
    try {
      ...
      if (contract && gameId) {
        ...
        toast.success(`Phase changed to ${currentPhase === 'Night' ? 'Day' : 'Night'}`)
      }
    } catch (error) {
                    ...
                    <AnimatePresence>
                      {players.map((player) => (
                        ...
                        >
                          ...
                          {player.isAlive && currentPhase === 'Night' && currentPlayer?.isModerator && (
                            <Button
